...
for i in `seq 1 ${n}`; do
    for method in "set" "get" "gets" "delete" "add" "incr" "decr" "replace" "append" "prepend"; do
        printf "[%d] command: %s =>\n" $i $method
        ${mcperf} --server=${server} --port=${port} --num-conns=${num_conns} --conn-rate=${conn_rate} --num-calls=${num_calls} --call-rate=${call_rate} --method=${method} --sizes=u${lb_size},${lb_size} >> /tmp/mcperf.out 2>&1
    done
